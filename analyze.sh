...
#
jq '.heyrovsky.barrier-1.00*.heyrovsky.barrier_beta' ../results/*/results.json > heyrovskys.txt
jq '.volmer.barrier-1.00*.volmer.barrier_beta' ../results/*/results.json > volmers.txt
...
jq '.heyrovsky.energy-1.00*.heyrovsky.beta' ../results/*/results.json > heyrovsky_energies_at-1V.txt
jq '.volmer.energy-1.00*.volmer.beta' ../results/*/results.json > volmer_energies_at-1V.txt
grep "\-1\.00 1.4" ../results/*/production.dat | awk '{print $4}' > pH14RHE-0.2_activity.txt
grep "\-1\.00 1.4" ../results/*/current.dat | awk '{print $3}' > pH14RHE-0.2_currents.txt
gnuplot lsv.plot
