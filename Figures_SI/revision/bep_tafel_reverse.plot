...
set xlabel '$\Delta \Omega_\mathrm{H}$ (eV)'
set ylabel '$\Delta \Omega_\mathrm{RevTaf}^\ddag$ (eV)'
set terminal epslatex color colortext size 4in,3in "cmss,10" standalone
set output "BEP_Reverse.tex"
set key top left 
set xtics 
set ytics 
set xzeroaxis
...
set print '/dev/null'
stats "<(paste vac_HBEs.txt tafel_energies.txt)" u 1:($2-2*$1) prefix 'BEP'
...
plot \
	"<(paste vac_HBEs.txt tafel_energies.txt metals.txt)" u 1:($2-2*$1):3 w labels point pt 7 offset char -1.5,0.2 notitle, \
