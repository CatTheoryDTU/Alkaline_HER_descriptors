...
set terminal epslatex color size 6in,4in "cmss"
...
fit f(x,y) "<paste PZCs.txt HBEs.txt i0s.txt" u 1:2:(1*($3-amps)) via a,b,c
fit piecef(x) "<paste HBEs.txt i0s.txt" using 1:(1*($2-amps)) via d,g,h
stats "<paste PZCs.txt HBEs.txt i0s.txt" using (f($1,$2)):(1*($3-amps)) prefix "DESC"
stats "<paste HBEs.txt i0s.txt" using (piecef($1)):(1*($2-amps)) prefix "HVOLCANO"
stats "<paste PZCs.txt i0s.txt" using ($1-4.44):(1*($2-amps)) prefix "PZC"
stats "<paste HBEs.txt i0s.txt" using 1:(1*($2-amps)) prefix "H"
...
plot \
	'<paste HBEs.txt i0s.txt metals.txt' u 1:(1*($2-amps)):(sprintf("%s",stringcolumn(3))) w labels point pt 7 offset char 0.5,0.5 notitle, \
...
plot \
	"<paste PZCs.txt HBEs.txt i0s.txt metals.txt" using (f($1,$2)):(1*($3-amps)):(sprintf("%s",stringcolumn(4))) w  labels point pt 7 offset char 1,0.5 notitle, \
