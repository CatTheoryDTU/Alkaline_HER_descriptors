...
set offset 1,1
#set fit quiet
set fit logfile 'newvolcano.log'
set print '/dev/null'
...
# plot r2 with coefficients, through a loop new .plot file
f2(x,y)=f22*(-0.58*(x-4.4)+y)+f23
g2(x,y)=g22*(-0.58*(x-4.4)+y)+g23
h2(x,y)=h22*(-0.58*(x-4.4)+y)+h23
fit f2(x,y) "<paste PZCs.txt HBEs.txt volmers.txt" u 1:2:3 via f22,f23
fit g2(x,y) "<paste PZCs.txt HBEs.txt heyrovskys.txt" u 1:2:3 via g22,g23
fit h2(x,y) "<paste PZCs.txt HBEs.txt tafels.txt" u 1:2:3 via h22,h23
...
set label 2 sprintf('\tiny{Heyrovsky, $R^2 = %1.2f$}',HEY_correlation**2) at -0.6,1.48 rotate by atan(3.6*g22/1.3)/pi*180-9 font subtitlefont textcolor lt 7 front
...
plot \
	"<paste PZCs.txt HBEs.txt volmers.txt numbers.txt" using (-0.58*($1-4.44)+$2):3:4 w points lc 'black' ps 2 pt variable notitle, \
	"<paste PZCs.txt HBEs.txt heyrovskys.txt numbers.txt" using (-0.58*($1-4.44)+$2):3:4 w points ps 2 pt variable lc 7 notitle, \
	"<paste PZCs.txt HBEs.txt tafels.txt numbers.txt" using (-0.58*($1-4.44)+$2):3:4 w  points ps 2 pt variable lc 6 notitle, \
