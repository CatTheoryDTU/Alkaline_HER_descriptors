set encoding utf8
ticsfont = "Helvetica,12"
titlefont = "Helvetica,22"
subtitlefont = "Helvetica,18"
#set terminal epslatex color size 6in,4in "cmss"
#set output "All_Activation_Energies.tex"
set terminal svg enhanced font titlefont size 1000,500
set output "All_Activation_Energies.svg"
load 'turbo.pal'
set xtics -2,1,0 font ticsfont nomirror #offset 0,graph 0.05 nomirror
set ytics font ticsfont nomirror
set offset 0.5,0.5
set yrange [0.0:2.0]
f(x)=a*(x-4.44)+b
set fit quiet
set fit logfile '/dev/null'
elements="Ag Au Cu Ir Ni Pd Pt Rh"
FILE = "numbers.txt"
array numbers[8]
stats FILE u (numbers[int($0+1)] = $1) 
array elements = ["Ag","Au","Cu","Ir","Ni","Pd","Pt","Rh"]
set multiplot layout 1,3 margins 0.05, 0.95, 0.25, 0.85 # title "Activation Energies"
set title font subtitlefont
set title "Volmer" #offset 0,graph -0.1
array coeffs_a[8]
array coeffs_b[8]
do for [i=1:8]{
	ele=elements[i]
	fit f(x) "../data/volmer/".ele."/barriers.txt" u 1:2 via a,b
	coeffs_a[i]=a
	coeffs_b[i]=b
}
set xlabel "Potential, V" #offset 0,screen 0.075
set ylabel "Activation Energy, eV"
set key at screen 0.8,screen 0.125 maxrows 2
set xrange [-2.5:0.5]
plot for [i=1:8] "../data/volmer/".elements[i]."/barriers.txt" u ($1-4.44):2 ps 2 pointtype numbers[i] title elements[i], \
     for [i=1:8] coeffs_a[i]*x+coeffs_b[i] lc i lw 2 dt 2 notitle
unset xrange
set ylabel ""
set xlabel "Potential, V "# offset 0,graph 0.12
set title "Heyrovsky"
array coeffs_a[8]
array coeffs_b[8]
do for [i=1:8]{
	fit f(x) "../data/heyrovsky/".elements[i]."/barriers.txt" u 1:2 via a,b
	coeffs_a[i]=a
	coeffs_b[i]=b
}
set xrange [-2.5:0.5]
plot for [i=1:8] "../data/heyrovsky/".elements[i]."/barriers.txt" u ($1-4.44):2 ps 2 pointtype numbers[i] title elements[i], \
     for [i=1:8] coeffs_a[i]*x+coeffs_b[i] lc i lw 2 dt 2 notitle
unset xrange
set xlabel "Potential, V "# offset 0,graph 0.12
set ylabel ""
set title "Tafel"
array coeffs_a[8]
array coeffs_b[8]
do for [i=1:8]{
	fit f(x) "../data/tafel/".elements[i]."/barriers.txt" u 1:2 via a,b
	coeffs_a[i]=a
	coeffs_b[i]=b
}
set xrange [-2.5:0.5]
plot for [i=1:8] "../data/tafel/".elements[i]."/barriers.txt" u ($1-4.44):2 ps 2 pointtype numbers[i] title elements[i], \
     for [i=1:8] coeffs_a[i]*x+coeffs_b[i] lc i lw 2 dt 2 notitle
unset xrange
unset multiplot
