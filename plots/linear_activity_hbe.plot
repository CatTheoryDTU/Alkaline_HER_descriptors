...
set print '/dev/null'
stats "<paste HBEs.txt i0s.txt" using 1:(1*($2)) prefix "H"
set xrange [-0.3:0.7]
set yrange [-10:2]
...
plot \
	'<paste HBEs.txt i0s.txt metals.txt' u 1:2:(sprintf("%s",stringcolumn(3))) every 2::0 w labels point pt 7 offset char -2.0,0.5 notitle, \
	'<paste HBEs.txt i0s.txt metals.txt' u 1:2:(sprintf("%s",stringcolumn(3))) every 2::1 w labels point pt 7 offset char 0.5,-1 notitle, \
