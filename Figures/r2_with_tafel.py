...
dband_centers=np.loadtxt('dbandcenters.txt')
dband_edges=np.loadtxt('dbandedges.txt')
colors=['g','k','b','g','m','b']
linestyles=['-','-','-','-.','--',':']
...
r2s_dcenter_volmer=[]
slopes_dcenter_volmer=[]
r2s_dcenter_heyrovsky=[]
slopes_dcenter_heyrovsky=[]

r2s_dedge_volmer=[]
slopes_dedge_volmer=[]
r2s_dedge_heyrovsky=[]
slopes_dedge_heyrovsky=[]
...
for potential in potentials:
    ...
    vslope, intercept, r_value, p_value, std_err = linregress(
            xdcenter,volmers+potential*volmer_betas)
    r2s_dcenter_volmer.append(r_value**2)
    slopes_dcenter_volmer.append(vslope)
    hslope, intercept, r_value, p_value, std_err = linregress(
            xdcenter,heyrovskys+potential*heyrovsky_betas)
    r2s_dcenter_heyrovsky.append(r_value**2)
    slopes_dcenter_heyrovsky.append(hslope)

    vslope, intercept, r_value, p_value, std_err = linregress(
            xdedge,volmers+potential*volmer_betas)
    r2s_dedge_volmer.append(r_value**2)
    slopes_dedge_volmer.append(vslope)
    hslope, intercept, r_value, p_value, std_err = linregress(
            xdedge,heyrovskys+potential*heyrovsky_betas)
    r2s_dedge_heyrovsky.append(r_value**2)
    slopes_dedge_heyrovsky.append(hslope)
...
ys=[r2s_pzc_volmer,r2s_hbe_volmer,r2s_htop_volmer,r2s_hbepzc_volmer,r2s_coup_volmer,r2s_hdiff_volmer,r2s_dcenter_volmer,r2s_dedge_volmer]
...
ys=[r2s_pzc_heyrovsky,r2s_hbe_heyrovsky,r2s_htop_heyrovsky,r2s_hbepzc_heyrovsky,r2s_coup_heyrovsky,r2s_hdiff_heyrovsky,r2s_dcenter_heyrovsky,r2s_dedge_heyrovsky]
...
ys=[]
for x in [xpzc,xhbe,xhtop,combined_hbepzc,xcoup,xhdiff,xdcenter,xdedge]:
...
np.savetxt('fig5c.txt',(np.array(ys).reshape(8,1)*np.ones((1,101))).T)
